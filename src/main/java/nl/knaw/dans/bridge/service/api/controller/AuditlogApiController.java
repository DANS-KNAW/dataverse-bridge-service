...
import nl.knaw.dans.bridge.service.generated.api.AuditlogApi;
...
public class AuditlogApiController implements AuditlogApi {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Plugin response", response = nl.knaw.dans.bridge.service.db.domain.ArchivingAuditLog.class, responseContainer = "List"),
        ...
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    ...
                    return new ResponseEntity<>(getObjectMapper().get().readValue(objectMapper.writeValueAsString(archivingAuditLogs), List.class), HttpStatus.OK);
                } catch (IOException e) {
    ...
    @ApiOperation(value = "", nickname = "getById", notes = "", response = nl.knaw.dans.bridge.service.db.domain.ArchivingAuditLog.class, tags={ "Archiving Auditlog", })
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Record Id to search", response = nl.knaw.dans.bridge.service.db.domain.ArchivingAuditLog.class),
            @ApiResponse(code = 400, message = "Invalid ID supplied"),
            @ApiResponse(code = 404, message = "Record not found") })
    @RequestMapping(value = "/auditlog/{id}",
            produces = { "application/json" },
            method = RequestMethod.GET)
    public ResponseEntity<nl.knaw.dans.bridge.service.db.domain.ArchivingAuditLog> getById(@ApiParam(value = "Record id",required=true) @PathVariable("id") Long id) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    ArchivingAuditLog dbArchivingAuditLog = archivingAuditlogDao.getById(id);
    ...
    @Override
    @ApiOperation(value = "", nickname = "getByState", notes = "", response = nl.knaw.dans.bridge.service.db.domain.ArchivingAuditLog.class, responseContainer = "List", tags={ "Archiving Auditlog", })
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Plugin response", response = nl.knaw.dans.bridge.service.db.domain.ArchivingAuditLog.class, responseContainer = "List"),
            @ApiResponse(code = 200, message = "unexpected error", response = Error.class) })
    @RequestMapping(value = "/auditlog/filtered-by-state/{state}",
            produces = { "application/json" },
            method = RequestMethod.GET)
    public ResponseEntity<List<nl.knaw.dans.bridge.service.db.domain.ArchivingAuditLog>> getByState(@ApiParam(value = "Record id",required=true) @PathVariable("state") String state) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
