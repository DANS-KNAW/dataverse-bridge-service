package nl.knaw.dans.bridge.service.db.dao;
...
public class ArchivingAuditlogDao {
     ...
     * Return the ArchivingAuditLog having the passed name.
     */
    public ArchivingAuditLog getBySrcurlSrcversionTargetiri(String srcMetadataUrl, String srcMetadataVersion, String darName) {
        Query q = entityManager.createQuery(
                "from ArchivingAuditLog where srcMetadataUrl = :srcMetadataUrl and srcMetadataVersion = :srcMetadataVersion and darName = :darName")
                .setParameter("srcMetadataUrl", srcMetadataUrl)
     ...
     * Return the ArchivingAuditLog having the passed name.
     */
    public ArchivingAuditLog getBySrcurlSrcversionTargetiriState(String srcMetadataUrl, String srcMetadataVersion, String darName, String state) {
        Query q = entityManager.createQuery(
                "from ArchivingAuditLog where srcMetadataUrl = :srcMetadataUrl and srcMetadataVersion = :srcMetadataVersion and darName = :darName")
                .setParameter("srcMetadataUrl", srcMetadataUrl)
                .setParameter("srcMetadataVersion", srcMetadataVersion)
