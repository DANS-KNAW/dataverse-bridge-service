<xsl:stylesheet
  ...
  <ddm:DDM>
    <ddm:profile>
      <dc:title>
        <xsl:value-of select="/map/map[@key='metadata']/array[@key='titles']/map/string[@key='title']/."/>
      </dc:title>
      <dcterms:description>
        <xsl:value-of select="/map/map[@key='metadata']/array[@key='descriptions']/map/string[@key='description']/."/>
      </dcterms:description>
      ...
      <!--</xsl:for-each>-->
      <ddm:created>
        <xsl:value-of select="format-dateTime(/map/string[@key='created']/.,'[Y0001]-[M01]-[D01]')"/>
      </ddm:created>
